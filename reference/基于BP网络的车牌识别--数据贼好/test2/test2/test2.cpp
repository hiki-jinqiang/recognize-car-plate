...
//训练函数的接口,输入矩阵， 预期输出矩阵，训练参数
	...
	bp2.save("mlp2.xml");/**/  //保存为mlp2.xml文件
//加载训练xml文件
	bp2.load("mlp2.xml");
	//提取目标特征
	...
	bp2.predict(zifu7,output7); //进行预测
     Point maxLoc0;  
        double maxVal0;  
...
//加载训练xml文件
   ...
   Point maxLoc;  
	...
	//神经网络的训练
/*	Mat inputs1(5100,560,CV_32FC1,Scalar(0));
	//初始化输入样本
	 ifstream file("muban2.txt");  
    int imgindex1 = 0;
    for (;imgindex1<5100;imgindex1++)  //表示文件流的结尾
  {  
        char txt_cont1[5100];//指针  
        file.getline(txt_cont1,5100); //获取字符串
        char imgfile1[5140]; 
        sprintf(imgfile1, "nzishu/%s", txt_cont1);  //连接字符串
		Mat src10 = imread(imgfile1);//注意读取的图片被视为是真彩色图像
	    cvtColor(src10,src10,CV_BGR2GRAY);
		threshold(src10,src10,0,255,CV_THRESH_OTSU+CV_THRESH_BINARY);
		Mat charfeature1(1,560,CV_32FC1,Scalar(0));
		features(src10, charfeature1);  
	for(int i=0;i<560;i++)
		{
			inputs1.at<float>(imgindex1,i)=charfeature1.at<float>(0,i);
		}
	}
...
//初始化输出样本
	Mat outputs1(5100,34,CV_32FC1,Scalar(0));
	CvMLData mlData1;
    mlData1.read_csv("output3.csv");//读取csv文件
   outputs1= Mat(mlData1.get_values(),true);
	
//训练函数的接口,输入矩阵， 预期输出矩阵，训练参数
	bp1.train(inputs1,outputs1,Mat(),Mat(), params1);
    bp1.save("mlp1.xml");
//加载训练xml文件*/
	bp1.load("mlp1.xml");
	//提取目标特征
...
{  
	...
	string name2="video/3test.avi";
	cout<<"识别图片按1；识别视频按其他整数"<<endl;
	int choose;
	cin>>choose;
  if(choose==1)
	   {  
